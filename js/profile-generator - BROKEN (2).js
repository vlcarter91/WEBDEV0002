// js/profile-generator.js

const fs = require("fs");
const path = require("path");

const ATHLETES_DIR = path.join(__dirname, "../images/athletes");
const PROFILES_DIR = path.join(ATHLETES_DIR, "profiles");

if (!fs.existsSync(PROFILES_DIR)) {
  fs.mkdirSync(PROFILES_DIR);
}

const template = (data, hasVideo) => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>${data.first} ${data.last} - Player Profile</title>
  <link rel="stylesheet" href="../../../css/styles.css" />
  <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@600&family=Roboto:wght@400&display=swap" rel="stylesheet" />
  <style>
    html, body {
      margin: 0;
      padding: 0;
      height: 100%;
      width: 100%;
      background: black;
    }

    .profile-layout {
      height: 100%;
      width: 100%;
    }

    .container05 {
      height: 100%;
      width: 100%;
      display: flex;
      justify-content: center;
      align-items: center;
      overflow: hidden;
      position: relative;
    }

    .container05 video,
    .container05 img {
      height: 100%;
      width: 100%;
      object-fit: cover;
      position: absolute;
      top: 0;
      left: 0;
    }

    .container05 img {
      display: none;
    }

    #unmuteButton {
      position: absolute;
      bottom: 20px;
      right: 20px;
      z-index: 10;
      background-color: rgba(0, 0, 0, 0.7);
      color: white;
      border: none;
      padding: 10px 15px;
      font-size: 1rem;
      border-radius: 8px;
      cursor: pointer;
      transition: opacity 0.3s ease;
    }

    #unmuteButton:hover {
      background-color: rgba(255, 255, 255, 0.2);
    }
  </style>
</head>
<body>

  <header>
    <nav aria-label="Main navigation">
      <ul>
        <li> | <a href="../../../index.html" class="active">Overview</a> | </li>
        <li><a href="../../../athletes.html">Our Athletes</a> | </li>
        <li><a href="../../../services.html">Our Services</a> | </li>
        <li><a href="../../../about.html">About Us</a> | </li>
      </ul>
    </nav>
  </header>

  <div class="profile-layout">
    <div class="left-stack">
      <div class="container01">
        <img src="../${data.filename}" alt="${data.first} ${data.last}" />
      </div>
      <div class="container02">Position: ${data.position}</div>
      <div class="container03">Birth Year: ${data.birth_year}</div>
    </div>

    <div class="container04">${data.first} ${data.last}</div>

    <div class="container05">
      ${hasVideo ? `
        <video id="introVideo" autoplay muted playsinline>
          <source src="../temp/${data.first}_${data.last}_intro.mp4" type="video/mp4" />
          Your browser does not support the video tag.
        </video>
        <button id="unmuteButton">🔊 Unmute</button>
      ` : ``}
      <img id="finalImage" src="../temp/${data.first}_${data.last}_Stats.jpg" alt="${data.first}_${data.last}" />
    </div>
  </div>

  <div id="site-footer">
    <footer>
      <div class="footer-title">
        <p>&copy; 2003 Emergent Technologies Media Solutions Inc.</p>
      </div>
      <div class="footer-subtext">
        <p>All rights reserved.</p>
      </div>
    </footer>
  </div>

  <script>
    const video = document.getElementById("introVideo");
    const image = document.getElementById("finalImage");
    const unmuteBtn = document.getElementById("unmuteButton");

    if (video) {
      video.addEventListener("ended", () => {
        video.style.display = "none";
        if (unmuteBtn) unmuteBtn.style.display = "none";
        image.style.display = "block";
      });

      if (unmuteBtn) {
        unmuteBtn.addEventListener("click", () => {
          video.muted = false;
          video.volume = 1.0;
          unmuteBtn.style.display = "none";
        });
      }
    }
  </script>
</body>
</html>`;



const files = fs.readdirSync(ATHLETES_DIR).filter(file =>
  /\.(jpg|jpeg|png|webp|gif)$/i.test(file)
);

files.forEach(file => {
  const parts = path.basename(file, path.extname(file)).split("_");
  if (parts.length < 4) return;

  const [first, last, position, birth_year, sport = "Hockey"] = parts;
  const videoFilename = `${first}_${last}_intro.mp4`;
  const videoPath = path.join(ATHLETES_DIR, videoFilename);
  const hasVideo = fs.existsSync(videoPath);

  const html = template({ first, last, position, birth_year, sport, filename: file }, hasVideo);

  const htmlFilename = path.join(PROFILES_DIR, `${path.basename(file, path.extname(file))}.html`);
  fs.writeFileSync(htmlFilename, html, "utf8");
  console.log(`✅ Created profile: ${htmlFilename}`);
});
